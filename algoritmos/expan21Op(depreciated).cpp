...
/*
	...
	./expand21  arqTreinoEntrada arqParesDominantes arqTesteEntrada arqTreinoSaida arqTesteSaida 
	* domMin supMin rMinDesbal justMinClases
	* o ultimo parametro indica se a expansao deve ocorrer somente nos documentos da classe minoritaria
	* no treino (1) ou se e para expandir qualquer documento que possua um par que atende domMin e 
	* supMin
*/

/*
  Definir o set que amazena as classes cujos pares de termos que predominam nessas classes serao 
  utilizados na expansão. Um par pode ser utilizado na expansão 
  de um documento somente se ele predominar em uma das classes desse set. As classes que participam
...
void expandeDocs(char* nomeArqEntrada, char* nomeArqSaida, int tamMaiorLinha, int numClasses, 
                 int numDocs, double domMin, double supMin, 
                 short int justMinClases);
//------
...
int main(int argc, char** argv) {
	...
	short int justMinClases = atoi(argv[9]);
	...
	expandeDocs(argv[1], argv[4], maxTermPorDoc, numClasses, numDocs, domMin, supMin, justMinClases );
	...
	expandeDocs(argv[3], argv[5], maxTermPorDoc,numClasses, numDocs, domMin, supMin,0);
...
void expandeDocs(char* nomeArqEntrada, char* nomeArqSaida, int tamMaiorLinha,  int numClasses, 
                 int numDocsTreino, double domMin, double supMin, short int justMinClases){
  fprintf(stderr, "domMin: %lf supMin: %lf\n",domMin, supMin);
	...
	double  numDocsPorClasse[numClasses];
	double  numDocsExp[numClasses];
	for(int i=0;i<numClasses;i++){
		numDocsPorClasse[i]=0.0;
		numDocsExp[i]=0.0;
	}
	while(fscanf(fin, "%d", &cat)!=EOF){
		fprintf(fout,"%d", cat);
		numDocsPorClasse[cat]++;
		c=fgetc(fin);
		mapTermPeso.clear();
		while(c!='\n'&& c!=EOF){
		...
		map <int,int> :: iterator it = suporteClasse.find(cat);
		if (it !=suporteClasse. end() || (it==suporteClasse.end()  && justMinClases==0)){
	...
	for(int i=0;i<numClasses;i++){
		fprintf(stderr,"Numero de documentos expandidos da classe: %d :%.2lf Percentagem de docs da classe que foram expandidos: %.2lf\n",
	       i, numDocsExp[i], numDocsExp[i]/numDocsPorClasse[i]);
	}       
