	...
	while(fscanf(fin, "%d", &cat)!=EOF){
		...
		do{
			...
			if(aux.df==0){
				...
				peso1=(double)freq*(double)aux.vetSuporteClass[aux.cat]; // tf * suporte da classe onde o termo ocorre mais 
				peso2=(double)aux.df;//df = numero de documentos onde o termo ocorre
				//*log(suporteClasse[aux.cat]);
		...
		// fprintf(fout," cl1= %d - cl2=%d",primClasse, segClasse);
		sort(vetTermos.begin(), vetTermos.end(),comparaPesosTermos);
		for (int k=0; k<vetTermos.size(); k++) {
			fprintf(fout," %d:%d:%.4lf", vetTermos[k].term,vetTermos[k].cat, vetTermos[k].peso);
		}

		sort(vetParesExpand.begin(), vetParesExpand.end(), comparaParId);
		for (int i=0; i<vetParesExpand.size(); i++) {
		  if(vetParesExpand[i].cat==primClasse||vetParesExpand[i].cat==segClasse)
